...
public class GoogleImageSearchCommand implements BotCommand {
    private static final Logger log = LoggerFactory.getLogger(GoogleImageSearchCommand.class);
    ...
    public String getShortHelp() {
        return "Lance une recherche sur Google Images";
    }
    ...
    public void runCommand(MessageReceivedEvent event, String[] parameters) throws IOException {
        String url = "https://www.googleapis.com/customsearch/v1?key=" + SecretConstants.GOOGLE_CUSTOM_SEARCH_API_KEY + "&gl=fr&hl=fr&num=5&cx=007315914358666863061:e0qfizftdqs" +
                "&q=" + URLEncoder.encode(parameters[0], StandardCharsets.UTF_8) + "&searchType=image";

        log.debug("Requête à Google : {}", url);

        try (InputStream listeResultats = ConnectionUtils.openStreamWithTimeout(url)) {
            JSONObject items = new JSONObject(new JSONTokener(listeResultats));
