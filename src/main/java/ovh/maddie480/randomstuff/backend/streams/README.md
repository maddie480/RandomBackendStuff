...
* `!poll` - runs a poll, users can vote using keywords in the chat. For example,
  `!poll "Are you bald?" Yes No "I'm not sure"`. Results are published
  on [https://maddie480.ovh/twitch-poll](https://maddie480.ovh/twitch-poll) and are refreshed every 5 seconds, so they
  can be shown on stream.

Also comes with [SHS Chat Control](features/SHSChatControl.java), the server side of a chat control mod for an obscure
game called
